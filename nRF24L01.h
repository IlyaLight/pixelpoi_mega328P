...
/*
		IRQ			—	выход прерывания. Когда все пучком, то нога находится 
						...
						Это видно из трех битов-флагов — RX_DR, TX_DS, MAX_RT. Как 
						только стало ясно какие	биты равны значению 1, их тут же надо 
						сбросить, записью 1. Иначе нога IRQ всегда будет лежать на 
						земле. Причем сбросить надо только те биты в которых значение 1. 
						Те в которых 0 писать ничего ненадо, ну 0 можно записать, но 
						не 1. Иначе IRQ опять упадет на землю.
...
/*0x00 	CONFIG Регистр настроек.*/
#define CONFIG          0x00
#define SET_CONFIG		((0 << MASK_RX_DR)|(0 << MASK_TX_DS)|(0 << MASK_MAX_RT)|(1 << EN_CRC)|(1 << CRCO)|(1 << PWR_UP )|(1 << PRIM_RX))							
...
/*0x01 	EN_AA 		Включает автоподтверждение приёма.
ENAA_Px - установка бита включает автоматическую отправку подтверждения приёма данных по 
соответствующему каналу.*/
#define EN_AA			0x01
//#define SET_EN_AA		(0 << ENAA_P5)|(0 << ENAA_P4)|(0 << ENAA_P3)|(0 << ENAA_P2)|(1 << ENAA_P1)|(0 << ENAA_P0)
#define	ENAA_P5			5		
#define	ENAA_P4			4
#define	ENAA_P3			3
#define	ENAA_P2			2
...
/*0x02 	EN_RXADDR 	Выбирает активные каналы приёмника.
...
#define SET_EN_RXADDR	(0 << ERX_P5)|(0 << ERX_P4)|(0 << ERX_P3)|(0 << ERX_P2)|(1 << ERX_P1)|(1 << ERX_P0)
#define	ERX_P5			5	
#define	ERX_P4			4					
#define	ERX_P3			3
#define	ERX_P2			2
...
/*0x03 	SETUP_AW		Задаёт длину поля адреса.
...
#define AW 				0
#if ADDR_LEN == 5
#define SET_SETUP_AW	3 << AW
#elif  ADDR_LEN == 4
#define SET_SETUP_AW	2 << AW
#elif  ADDR_LEN == 3
#define SET_SETUP_AW	1 << AW
#else
#error	Incorrect value of ADDR_LEN
#endif

/*0x04 	SETUP_RETR	Настройка параметров автоматического повтора отправки.
...
#define SET_SETUP_RETR	(0 << ADR)|(6 << ARC)
#define ADR 			4
#define	ARC 			0

/*0x05 	RF_CH		Регистр задаёт номер радиоканала 
...
#define RF_CH			0x05
#define SET_CHANAL		76

/*0x06 	RF_SETUP 	Задаёт настройки радиоканала.*/
#define RF_SETUP		0x06
#define SET_RF_SETUP	(0<<CONT_WAVE)|(0<<RF_DR_LOW)|(0<<PLL_LOCK)|(0<<RF_DR)|(3<<RF_PWR)
...
#define RX_P_NO			1		/*три бита кодирующие номер канала, пакет по которому доступен в 
								...
								пуста.*/
#define TX_FULL			0		/*значение 1 этого бита показывает, что в очереди передатчика нет 
								свободных слотов.*/

/*0x08 	OBSERVE_TX	Регистр контроля передатчика*/
...
/*0x09	RPD			Оценка мощности принимаемого сигнала*/
...
//#define SET_RX_ADDR_P0	SET_TX_ADDR
/*0x0B 	RX_ADDR_P1 
...
#define SET_RX_ADDR_P1	0xE8, 0xE8, 0xF0, 0xF0, 0xE1
...
//#define SET_RX_ADDR_P2	0xC3	
#define RX_ADDR_P3		0x0D
//#define SET_RX_ADDR_P3	0xC4
#define RX_ADDR_P4		0x0E
//#define SET_RX_ADDR_P4	0XC5
#define RX_ADDR_P5		0x0F
//#define SET_RX_ADDR_P5	0XC5
...
Значение регистра по умолчанию: 0xE7E7E7E7E7*/
#define TX_ADDR			0x10
//#define SET_TX_ADDR		0xC2, 0xC2, 0xC2, 0xC2, 0xC2
...
//#define SET_RX_PW_P0	0
#define RX_PW_P1		0x12
#define SET_RX_PW_P1	20
#define RX_PW_P2		0x13
//#define SET_RX_PW_P2	0
#define RX_PW_P3		0x14
//#define SET_RX_PW_P3	0
#define RX_PW_P4		0x15
//#define SET_RX_PW_P4	0
#define RX_PW_P5		0x16
//#define SET_RX_PW_P5	0

/*0x17 	FIFO_STATUS Состояние очередей FIFO приёмника и передатчика*/
...
#define RX_FULL			1		/*Флаг переполнения FIFO очереди приёмника: 0 - есть свободное 
								место в очереди; 1 - очередь переполнена.*/
#define RX_EMPTY		0		/*Флаг освобождения FIFO очереди приёмника: 0 - в очереди есть 
								данные; 1 - очередь пуста.*/

/*0x1C 	DYNPD Разрешение использования пакетов произвольной длины.
...
#define DYNPD			0x1C
#define SET_DYNPD		(0 << DPL_P5)|(0 << DPL_P4)|(0 << DPL_P3)|(0 << DPL_P2) | (1 << DPL_P1) | (1 << DPL_P0)
#define DPL_P5			5
#define DPL_P4			4
#define DPL_P3			3
#define DPL_P2			2
...
/*0x1D 	FEATURE	Регистр опций*/
#define FEATURE 		0x1D
#define SET_FEATURE		(1 << EN_DPL)|(1 << EN_ACK_PAY)|(0 << EN_DYN_ACK)
...
#define R_RX_PL_WID	        0b01100000		// 1				Read RX payload width for the top
											//					R_RX_PAYLOAD in the RX FIFO.
											//					Note: Flush RX FIFO if the read value is larger
											//					than 32 bytes.
										
#define W_ACK_PAYLOAD		0b10101000		// 1 to 32			Used in RX mode.
